...
function RpcClient(opts) {
  ...
  generateRPCMethods(RpcClient, RpcClient.chain[opts.chain].callspec, rpc, opts.chain);
...
function rpc(request, callback) {
  ...
  var errorMessage = 'Chain JSON-RPC: ';
  ...
  var req = this.protocol.request(options, function(res) {
    ...
    res.on('end', function() {
      ...
      parsedBuf.error = (parsedBuf.error) ? parsedBuf.error : null
...
RpcClient.chain = {}
RpcClient.chain.ethereum = {}
RpcClient.chain.ethereum.callspec = {
  web3_clientVersion : '',
  web3_sha3: 'str',
  net_version: '',
  net_peerCount: '',
  ...
  db_putString: 'str str str',
  db_getString: 'str str',
  db_putHex: 'str str str',
  db_getHex: 'str str',
  shh_post: 'obj',
  shh_version: '',
  shh_newIdentity: '',
...
RpcClient.chain.bitcoin = {}
RpcClient.chain.bitcoin.callspec = {
...
function generateRPCMethods(constructor, apiCalls, rpc, chain) {
  ...
  for(var k in apiCalls) {
    ...
    var methodName = (chain == "ethereum") ? k : k.toLowerCase();
